...
:memo:如何理解操作系统中的C程序，裸机上的C程序(像操作系统)？
C程序要经过预处理，编译，汇编，链接才能得到可执行文件。操作系统中的C程序过程往往是，Shell得到命令后，操作系统先fork()再execve()再用加载器识别文件，做出内存映射，执行ld-linux-x86-64.so执行_start函数最后执行main。裸机上的C程序可以认为是CPU Reset后，Firmware读取MBR中的Boot loader，接着执行'kernel'程序的_start函数执行(操作系统也是一个C程序，引导加载程序)。综上，目前只要知道main函数之前(操作系统也是如此)初始化好了C程序环境并能够使用一点库函数足矣。

:memo:如何理解系统调用fork，execve，mmap？
操作系统运行起来，创建第一个进程init，然后用它创建出更多进程。系统调用fork复刻一个新进程，系统调用execve初始化进程状态，系统调用mmap映射进程空间。系统调用fork和execve之间可以添加改变程序运行环境的代码，这为shell提供了便利(典型的例子是shell中的管道命令需要在fork创建子进程后改变输入输出)。

## 小结

:memo:硬件，内核，shell，终端，操作系统，系统调用，libc，应用程序之间的关系？ 
内核有进程管理，内存管理，文件管理等作用。shell提供了用户和内核交互的接口以保护内核。终端(ctrl+C是发送的信号)是计算机的输入输出设备。操作系统主要由C和汇编写成。系统调用是程序访问内核程序的接口。libc是计算和系统调用的进一步封装。封装操作系统的输入输出对象(文件描述符)，封装更多进程和操作系统功能(比如报错函数)，封装地址空间的分配(malloc和free)。进一步封装C++的编译器，封装标准C++库，封装python解释器Cpython，当然不满足C也有人封装了Go。基于这些有了各种各样的应用程序，最后编程开发就是STFW,RTFM,RTFSC的事了。
